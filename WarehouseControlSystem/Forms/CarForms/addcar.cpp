...
AddCar::AddCar(const QStringList &list,QString flag, QWidget *parent = nullptr):DialogAbstractClass(list,flag,parent)
{
...
void AddCar::onYesBtnClicked()
{
    ...
    {
         sql = QString("INSERT t_hwcomm_info SET ID = '%1',type = '1',childtype = '1', protype = 2, port = '%2',name = '%3';")
            .arg(deviceStruct.deviceNum.toInt()).arg(deviceStruct.port).arg(deviceStruct.deviceIp);
         sql += QString("INSERT t_device_info SET id = '%1', devicePort = '%2',deviceIp = '%3';")
                 .arg(deviceStruct.deviceNum.toInt()).arg(deviceStruct.port).arg(deviceStruct.deviceIp);
    }
    else
    {
        ...
        sql += QString("UPDATE t_device_info SET deviceIp ='%1', devicePort = '%2' WHERE id = '%3';")\
                .arg(deviceStruct.deviceIp).arg(deviceStruct.port).arg(deviceStruct.deviceNum.toInt());
    }
