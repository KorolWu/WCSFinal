...
AgvForm::AgvForm(int width, int height, QWidget *parent) : BaseFrom(parent)
{
    ...
    p_add_btn->move(m_width/158,heitht_fristLine);
    ...
    p_table_view->resize(m_width,m_height);
    ...
    QStringList header;
    header<<"ID"<<"穿梭车编号"<<"通讯地址"<<"端口号"<<"类型"<<"电量"<<"状态"<<"编辑";
    for(int i = 0 ;i <header.size();i++)
    {
        model->setHeaderData(i, Qt::Horizontal, header.at(i));
        p_table_view->setColumnWidth(i,m_width/header.size());
    }

    //    p_table_view->setColumnWidth(0,this->m_width/20*9);
    //    p_table_view->setColumnWidth(1,this->m_width/20*3);
    //    p_table_view->setColumnWidth(2,this->m_width/20*2);
    //    p_table_view->setColumnWidth(3,this->m_width/20*4);
    //    p_table_view->setColumnWidth(4,this->m_width/20*2);
    //    p_table_view->setColumnWidth(5,this->m_width/20*3);
    //    p_table_view->setColumnWidth(6,this->m_width/20*2);
    ...
    for(int j = 0; j< 7 ;j++)
    {
        ...
        for(int i = 0;i<row_count;i++)
        {
            if(j == 0)
                ...
                model->setItem(i,j,new QStandardItem(QString("%1").arg(it.value().carId)));
            else if(j == 2)
...
void AgvForm::onAddClicked()
{
    ...
    CommSetWidget *d = new CommSetWidget(0,"Insert",this);
    //connect(d,&AddCar::insert_emit,this,&AgvForm::refreshTable);
    d->move(r.width()/2-d->width(),r.height()/2-d->height());
    d->show();
...
void AgvForm::onDeleteClicked()
{
    ...
    CRUDBaseOperation::getInstance()->queryUseStr(sql_str);
