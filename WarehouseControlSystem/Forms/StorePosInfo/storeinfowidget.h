...
class StoreInfoWidget  : public QWidget
{
    Q_OBJECT
public:
    explicit StoreInfoWidget(QWidget *parent = nullptr);
    ~StoreInfoWidget();
private:
...
private:
    ...
    void WriteNTstoreposinfo( int box_boxfixed , int roadx, int y, int Spacerpost,int k,double z);//2021 02-20 新增方便创建南通立体库的数据信息
    void WriteNTstackerposdinfoOld(int layermax,int colmunmax);
    void WriteNTstackerposdinfonew(int layermax,int colmunmax);
public slots:
