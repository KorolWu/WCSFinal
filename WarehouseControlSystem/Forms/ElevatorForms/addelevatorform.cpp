...
void AddElevatorForm::onYesBtnClicked()
{
    ...
    if(m_sql_flag == "Insert")
    {
     ...
     hw_sql = QString("INSERT t_hwcomm_info SET ID = '%1',type = '4',protype = 7, port = '%2',name = '%3';")
        .arg(QString(list[0]).toInt()).arg(list[3]).arg(list[2]);
    }
    else
    {
        sql = QString("UPDATE t_elevator SET p_address='%1',type='%2',port='%4',near_position='%5',position='%6' WHERE ip_address='%7'")
              .arg(list[2]).arg(list[1]).arg(list[3]).arg(list[4]).arg(list[5]).arg(list[0]);
