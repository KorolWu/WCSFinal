...
MonitorUI::MonitorUI(QWidget *parent):QWidget(parent)
{
    ...
    // m_refreshbtn->setIcon(QIcon(":/resouse/Image/Refresh.png"));background-repeat: no-repeat;background-position:left;border:none;color:rgb(200, 200, 200);
    ...
    m_caritem->SetIndexID("0");
    //初始化小车的位置加入那个画布当中
    if(m_cursceneMap.contains(m_carpos.z) )
    {
      m_cursceneMap[m_carpos.z]->addItem(m_caritem);
    }
    ...
    m_timer->start(100);
    //elevator 升降机 每一层
    m_curelevatorLayer = 1;
    for(auto it = m_cursceneMap.begin();it != m_cursceneMap.end();++it)
    {
        StoreItem * elevatorItem = new StoreItem(-30,(520+500+1258)/k,30,(520+903+500+1258)/k);
        elevatorItem->SetIndexID("100");
        elevatorItem->SetStoreSate(0);
        elevatorItem->SetText("升降机");
        it.value()->addItem(elevatorItem);
    }
}
...
///刷新数据库的地图数据进行重建 刷新数据的时候 需要停掉刷新小车的数据和电梯的数据
void MonitorUI::updateUIbyData()
{
//    //QPropertyAnimation
//    QGraphicsItemAnimation *animation = new QGraphicsItemAnimation;
//    animation->setItem(m_caritem);
...
//    //animation->setPosAt(1,QPointF(m_caritem->x(), -200));
//    timeline->start();
    ...
    //显示画布首页的状态
    //第一种方式删除对应的item
    bool caronscene = false;
    m_carpos = GetCarPos(0); //默认ID等于99
    if(m_cursceneMap.contains(m_carpos.z))
    {
        caronscene = true;
    }
    for(auto it = m_cursceneMap.begin(); it != m_cursceneMap.end(); )
    {
    ...
    //    {
    //        QGraphicsScene *Scene =  it.value();
    //         QRectF Rect( m_minX/k-50,m_minY/k-50,(m_X-m_minX)/k+100,(m_Y-m_minY)/k+100);
...
void MonitorUI::SetUIDataItem()
{
    ...
    for(int i =0; i < m_laylist.size(); ++i )
    {
        ...
        //curlay->setSceneRect(m_minX/k-50,m_minY/k-50,5000,2000);
       // qDebug()<<"curscencerect"<<m_minX/k-50<<m_minY/k-50<<(m_X-m_minX)/k+100<<(m_Y-m_minY)/k+100 <<m_X-m_minX << m_Y-m_minY ;
        curlay->setItemIndexMethod(QGraphicsScene::NoIndex);
        ...
        if(m_caritem->GetIndexID() == QString::number(it.key()))
        {
            ...
            if(xdiffvalue != 0 )
            {
                ...
                m_carpos.x = it.value().deveceStatus.carCurrentPosion.x;
                 qDebug()<<"更新小车X状态差值 生成数组："<<xdiffvalue<<"-"<<m_xpathppos;
                break;
            }
            double ydiffvalue = it.value().deveceStatus.carCurrentPosion.y-m_carpos.y;
            //qDebug()<<"update pos y 检测到 y值出现了差距" <<it.value().deveceStatus.carCurrentPosion.y;
            if(ydiffvalue != 0 )
            {
                //生成y计算轨迹
                m_ypathppos.clear();
...
///
/// \brief MonitorUI::UpdateCarPosInScene
///小车层数更新
void MonitorUI::UpdateCarPosInScene()
{
    auto it =  Myconfig::GetInstance()->m_CarMap.begin();
    for(;it!= Myconfig::GetInstance()->m_CarMap.end();++it)
    {
        ...
        {
            //增加Z层数判断2020/09/28
            if(m_carpos.z != it.value().deveceStatus.carCurrentPosion.z)
            {
                //qDebug()<<"layer:"<<it.key() << it.value().deveceStatus.carCurrentPosion.z << m_carpos.z;
                double z = m_carpos.z;
                //画布场景变化更新
                if(m_cursceneMap.contains(z))
                {
                    ...
                    if(Scene)
                    {
                        foreach(QGraphicsItem *mGraphicsItem, Scene-> items())
                        {
                            ...
                            if(!FindItem)
                                continue;
                            QString id = FindItem->GetIndexID();
                            if(id == m_caritem->GetIndexID())
                            {
                                Scene->removeItem(FindItem);
                                //  FindItem->deleteLater();
                                break;
                            }
                        }
                        //增加到新的画布中
                        if(m_cursceneMap.contains(it.value().deveceStatus.carCurrentPosion.z ))
                        {
                            m_cursceneMap[it.value().deveceStatus.carCurrentPosion.z ]->addItem(m_caritem);
                        }
                    }
                }
            }
...
void MonitorUI::UpDateElevatorlayer()
{
    auto it =  Myconfig::GetInstance()->m_elevatorMap.begin();
    for(;it!= Myconfig::GetInstance()->m_elevatorMap.end();++it)
    {
        if(it.key() == 100)
        {
            if(it.value().status.curruntLayer != m_curelevatorLayer) //层数发生了变化
            {//找到之前的层数画布
                if(m_cursceneMap.contains(m_curelevatorLayer))
                {
                    QGraphicsScene *Scene =  m_cursceneMap[m_curelevatorLayer];
                    if(Scene)
                    {
                        foreach(QGraphicsItem *mGraphicsItem, Scene-> items())
                        {
                            ...
                            if(id == "100")
                            {
                                FindItem->SetStoreSate(0);
                            }
                        }
                    }
                }
                if(m_cursceneMap.contains(it.value().status.curruntLayer))
                {
                    QGraphicsScene *Scene =  m_cursceneMap[it.value().status.curruntLayer];
                    if(Scene)
                    {
                        foreach(QGraphicsItem *mGraphicsItem, Scene-> items())
                        {
                            ...
                            if(id == "100")
                            {
                                FindItem->SetStoreSate(10);
                            }
                        }
                    }
                }
                m_curelevatorLayer = it.value().status.curruntLayer;
            }
            break;
        }
    }
...
void MonitorUI::slotCarPathsimulation()
{
    if(m_xpathppos.size() > cnt )
    {
        ...
        qDebug()<<" 轨迹 在更新了 x:movvalue"<<m_carpos.x/k<<m_carpos.x/k<<m_xpathppos[cnt];
        cnt++;
    }
    else{
    ...
    if(m_ypathppos.size() > cnt)
    {
        ...
        // qDebug()<<"indexid:"<<m_caritem->GetIndexID() << m_caritem->GetText();
        m_caritem->setPos(m_caritem->x(),m_ypathppos[cnt]);
        qDebug()<<"轨迹 在更新y:movvalue"<<m_carpos.x/k<<m_carpos.y/k<<m_ypathppos[cnt];
        cnt++;
    }
    else{
