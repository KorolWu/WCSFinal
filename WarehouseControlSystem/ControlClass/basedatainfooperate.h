...
public:
    ...
    ///根据仓位信息获得对应料箱编号
    /// 返回为空，那么仓位无料箱处于空闲状态
    static bool  GetBoxinfo(QString Whloctioninfo,QString &boxnfo)
    {
        QMutexLocker locker(&Myconfig::GetInstance()->m_rmutex);
    ...
    ////
    /// \brief GetKposInfoByIDnbr
    /// \param id 货架号
    /// \param posstru
    /// \return  flase 仓库里面没有这个货架号
    ///
    static bool GetKposInfoByIDnbr(QString id,KPosition &posstru)
    {
        bool check = false;
        QMutexLocker locker(&Myconfig::GetInstance()->m_rmutex);
        auto it = Myconfig::GetInstance()->m_storeinfoMap.begin();
        for(;it !=Myconfig::GetInstance()->m_storeinfoMap.end(); ++it)
        {
            if(it.key() == id )
            {
                posstru.x = static_cast<int>(it.value().coordx);
                posstru.y = static_cast<int>(it.value().coordy);
                posstru.z = static_cast<int>(it.value().coordz);
                QStringList keylist = it.key().split("-");
                if(keylist.size() == 5)
                {
                    QString dirstateinfo = keylist[2];
    ...
    ///
    /// \brief UpdateWarehousBoxState
    /// \param id
    /// \param boxnbr
    /// \return
    ///
    static bool UpdateWarehousBoxState(QString id,QString boxnbr)
    {

        bool check = false;
        QMutexLocker locker(&Myconfig::GetInstance()->m_mutex);
        if(Myconfig::GetInstance()->m_storeinfoMap.contains(id))
        {
            Myconfig::GetInstance()->m_storeinfoMap[id].update = true;
            if(boxnbr != "")
            {
                Myconfig::GetInstance()->m_storeinfoMap[id].storestat = 1;
            }
            else{
                Myconfig::GetInstance()->m_storeinfoMap[id].storestat = 0;
            }
            check = true;
            int size = boxnbr.length();
            if(size > 64)
            {
                size = 64;
            }
            memset(Myconfig::GetInstance()->m_storeinfoMap[id].boxnbr,0,64);
            memcpy(Myconfig::GetInstance()->m_storeinfoMap[id].boxnbr,boxnbr.toStdString().c_str(),size);
        }
        return  check;
    }
};
