...
bool BaseDevice::init()
{
    ...
    {
        if(initResult)
            Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.isOnline = true;
        else
        {
            Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.isOnline = false;
     ...
     if(Myconfig::GetInstance()->m_CarMap.contains(m_carNum))
     {
         ...
         //{
            ...
            //{
                ...
                Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.model = r.v;
                memcpy(r.c,c+8,2);
                char byte = r.c[0];
                byte = r.c[0] >> 1 & 0x01; // =1 alram
                if(byte == 1)
                    Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.err_code = 1;
                else
                    Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.err_code = 0;
                byte = r.c[0] >> 2 & 0x01; // =1 battery low
                if(byte == 1)
                {
                    Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.batter = 20;
                    //qDebug()<<m_ip<<"batter"<< 20;
                }
                else
                {
                    Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.batter = 80;
                     //qDebug()<<m_ip<<"batter"<< 80;
                }
                byte = r.c[0] >> 3 & 0x01; // =1 standby can get order
                if(byte == 1 )
                    Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.status = 1;
                byte = r.c[0] >> 4 & 0x01; // =1 do working can`t get order
                if(byte == 1 )
                    Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.status = 2;
                ...
                Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.box_status = 0;
                byte = c[8] >> 1 & 0x01;  // 左有货
                if(byte == 1)
                    Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.box_status = 1;
                byte = c[8] >> 1 & 0x01;  //右有货
                if(byte == 1)
                    Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.box_status = 2;
                byte = c[8] >> 1 & 0x01;  //台上有货
                if(byte == 1)
                    Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.box_status = 3;
...
{
    if(Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.isOnline != false)
        Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.isOnline = false;
    ReadTableData mysql;
    ALARMINFOSTRU arm;
    arm.alarminfo = "设备 "+m_ip+" 掉线";
    arm.alarmlevel = 0;
    arm.carcoordx = Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.carCurrentPosion.x;
    arm.carcoordy = Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.carCurrentPosion.y;
    arm.carcoordz = Myconfig::GetInstance()->m_CarMap[m_carNum].deveceStatus.carCurrentPosion.z;
    arm.deviceid = m_ip;
    QString err;
...
void BaseDevice::unLockLayer(int layer)
{
    if(Myconfig::GetInstance()->m_layerStatusMap.contains(layer))
    {
        ...
        Myconfig::GetInstance()->m_layerStatusMap[layer].layerstate = LayerState::Free;
    }
