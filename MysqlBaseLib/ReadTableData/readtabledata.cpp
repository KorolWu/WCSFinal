...
void ReadTableData::readt_codePosition()
{
    QMutexLocker locker(&Myconfig::GetInstance()->m_mutex);
    QString sql = "SELECT * FROM t_codePosition;";
    QSqlQuery query = CRUDBaseOperation::getInstance()->queryDb(sql);
    int code = -1;
    KPosition p;
    while (query.next()) {
        code = query.value(0).toInt();
        p.z = query.value(1).toInt();
        p.x = query.value(2).toInt();
        p.y = query.value(3).toInt();
        Myconfig::GetInstance()->m_codeResiveMap.insert(code,p);
    }
}
///
...
bool ReadTableData::WriteNTStoreposinfotoDataBase(QMap<QString, StorePosInfoStru> storeposInfoMap, QString &errorinfo, QString tablename)
{
    QStringList names;
    names<<"idNbr"<<"type"<<"coordx"<<"coordy"<<"coordz" << "directionstate"<<"boxnbr"<<"storestat"<<"storepri"<<"unused";
    QList<QVariantList> values;
...
void ReadTableData::Readt_hwcomm_infotable()
{
    ...
    while (query.next()) {
        ...
        if(type == HWDEVICETYPE::RUNNER)
        {
            ...
            stru.port = query.value("port").toInt();
            stru.deviceIp = name;
            stru.deviceNum = QString::number(Id);
            if(Myconfig::GetInstance()->m_RunnerMap.contains(Id))
            {
                Myconfig::GetInstance()->m_RunnerMap[Id] = stru;
            }
            else{
                Myconfig::GetInstance()->m_RunnerMap.insert(Id,stru);
            }            
        }
        if(type == HWDEVICETYPE::STACKER)
        {
            StackerInfo stakerstru;
            Myconfig::GetInstance()->m_stackerMap.insert(Id,stakerstru);
        }
    }
}
///
